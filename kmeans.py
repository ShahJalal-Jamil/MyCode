...
X, Y, XTest, YTest, dataText, t = preProcess.preProcessDataUnsupervised(True)
...
# print(culster)
# print(listY)
# plt.scatter(culster, listY, c='black')
# plt.show()
...
YTextMisClassifyMain = pd.DataFrame()
int
track = 0
for nRow in ne:
    if nRow == True:
        ...
        YTextMisClassifyMain = YTextMisClassifyMain.append(YPredict.iloc[[track]])
    else:
...
i = 0
listY = list()
for im in XTextMisClassify.index:
    listY.append(i)
    i = i + 1
plt.scatter(YTextMisClassifyMain, listY, c='black')
plt.show()
...
# dendrogram = sch.dendrogram(sch.linkage(XTextMisClassify, method='ward'))  # create clusters
...
# plt.title('Cluster of custome reviews')
# plt.xlabel('Reviews')
...
#
# cluster = AgglomerativeClustering(n_clusters=2, affinity='euclidean', linkage='ward')
# all_predictions = cluster.fit_predict(X)
# from sklearn import metrics
#
# accuracy = metrics.accuracy_score(Y, all_predictions)
...
# ms = MeanShift()
# all_predictions = ms.fit_predict(X)
# accuracy = metrics.accuracy_score(Y, all_predictions)
# print(accuracy * 100, '%')
# print(metrics.classification_report(Y, all_predictions))
# print(metrics.confusion_matrix(Y, all_predictions))
